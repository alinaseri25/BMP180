...
BMP180::BMP180(I2C_HandleTypeDef *_hi2c1)
{
	hi2c1 = _hi2c1;
}

BMP180_RESULT BMP180::Begin(void)
{
	BMP180_Reset();
	osDelay(15);
...
void BMP180::BMP180_WriteReg(uint8_t reg, uint8_t value) {
	uint8_t buf[2];

	buf[0] = reg;
	buf[1] = value;
	HAL_I2C_Master_Transmit(hi2c1,BMP180_ADDR,buf,2,10);
}
...
// return:
//   register value
uint8_t BMP180::BMP180_ReadReg(uint8_t reg) {
	uint8_t value = 0; // Initialize value in case of I2C timeout

	// Send register address
	HAL_I2C_Master_Transmit(hi2c1,BMP180_ADDR,&reg,1,10);
	// Read register value
	HAL_I2C_Master_Receive(hi2c1,BMP180_ADDR,&value,1,10);
...
BMP180_RESULT BMP180::BMP180_Check(void) {
	uint8_t value;
...
uint8_t BMP180::BMP180_GetVersion(void) {
	return BMP180_ReadReg(BMP180_VERSION_REG);
}

// Read calibration registers
void BMP180::BMP180_ReadCalibration(void) {
	uint8_t buffer[BMP180_PROM_DATA_LEN];

	buffer[0] = BMP180_PROM_START_ADDR;
	// Send calibration first register address
	HAL_I2C_Master_Transmit(hi2c1,BMP180_ADDR,buffer,1,10);
	HAL_I2C_Master_Receive(hi2c1,BMP180_ADDR,buffer,BMP180_PROM_DATA_LEN,100);
...
BMP180_RESULT BMP180::BMP180_Read_UT(uint16_t *UT) {
	uint8_t buf[2];

	BMP180_WriteReg(BMP180_CTRL_MEAS_REG,BMP180_T_MEASURE);
	osDelay(6); // Wait for 4.5ms by datasheet

	buf[0] = BMP180_ADC_OUT_MSB_REG;
	// Send ADC MSB register address
	if (HAL_I2C_Master_Transmit(hi2c1,BMP180_ADDR,buf,1,10) != HAL_OK) {
		*UT = 0;
		return BMP180_ERROR;
	}
	// Read ADC MSB and LSB
	if (HAL_I2C_Master_Receive(hi2c1,BMP180_ADDR,buf,2,100)  == HAL_OK) {
		*UT = (buf[0] << 8) | buf[1];
		return BMP180_SUCCESS;
	} else {
		*UT = 0;
...
// return:
//   BMP180_ERROR if it was an I2C timeout, BMP180_SUCCESS otherwise
BMP180_RESULT BMP180::BMP180_Read_PT(uint32_t *PT, uint8_t oss) {
	uint8_t buf[3];

	// Start pressure measurement
	BMP180_WriteReg(BMP180_CTRL_MEAS_REG,BMP_OSS[oss].OSS_cmd);
	osDelay(BMP_OSS[oss].OSS_delay);
	...
	if (HAL_I2C_Master_Receive(hi2c1,BMP180_ADDR,buf,3,100) == HAL_OK) {
...
// return:
//   BMP180_ERROR if it was an I2C timeout, BMP180_SUCCESS otherwise
BMP180_RESULT BMP180::BMP180_Read_PT_3AVG(uint32_t *UP) {
	uint32_t RAW = 0;
	uint8_t i = 0;
	uint8_t cntr = 0;
...
int16_t BMP180::BMP180_Calc_RT(uint16_t UT) {
...
int32_t BMP180::BMP180_Calc_RP(uint32_t UP, uint8_t oss) {
	int32_t B3,B6,X3,p;
	uint32_t B4,B7;
...
int16_t BMP180::BMP180_GetTemperature(void) {
	uint16_t UT;
...
int32_t BMP180::BMP180_GetPressure(BMP180_Mode_TypeDef mode) {
	uint32_t UP;
	uint16_t UT;
...
// input:
//   RT - pointer to temperature value * 10
//   RP - pointer to pressure value
//   mode - BMP180 measure accuracy mode
// return:
//   BMP180_ERROR if it was an I2C timeout, BMP180_SUCCESS otherwise
BMP180_RESULT BMP180::BMP180_GetReadings(int16_t *RT, int32_t *RP, BMP180_Mode_TypeDef mode) {
	uint32_t UP;
	uint16_t UT;
...
int32_t BMP180::BMP180_hPa_to_mmHg(int32_t hPa) {
...
// return:
//   altitude (meters * 10)
int32_t BMP180::BMP180_hPa_to_Altitude(int32_t hPa) {
//	i = (uint32_t)(4433000 * (1 - powf(pressure_history[0]/101325.0,0.19029495)));
	return (((float)(((745 * (11390 - (hPa / 10))) / 256 + 46597) * (11390 - (hPa / 10)))) / 65536 - 966) * 10;
}
